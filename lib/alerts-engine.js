...
// ALERT_CODE_104: verify if node is in syncing state and not ready for production yet
const ALERT_CODE_104 = 104
...
export class AlertsEngine {
  ...
  async run(currentHealthCheck) {
    ...
    // ALERT_CODE_104
    if (currentHealthCheck.status === 'success' && previousHealthCheck === 'success') {
      if (currentHealthCheck.record.health !== undefined && previousHealthCheck.record.health !== undefined) {
        if (currentHealthCheck.record.health.isSyncing && previousHealthCheck.record.health.isSyncing) {
          console.debug('Creating new [alert] job for', currentHealthCheck.memberId, currentHealthCheck.serviceId)
          this.queue.add(
            cfg.alertsEngine.queueName,
            {
              code: ALERT_CODE_104,
              severity: SEVERITY_LOW,
              message: `Node is in syncing state for at least two health checks [#${currentHealthCheck.id}, #${previousHealthCheck.id}]`,
              memberId: currentHealthCheck.memberId,
              serviceId: currentHealthCheck.serviceId,
              healthCheckId: currentHealthCheck.id,
              healthChecks: [currentHealthCheck, previousHealthCheck],
